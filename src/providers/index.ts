...
const web3 = new Web3()
const SUPPORTED_L2_CHAINS = [ChainId.ARBITRUM_ONE, ChainId.POLYGON, ChainId.OPTIMISM]
...
export async function buildList(
  ...
  for (const l1Token of l1TokenList.tokens) {
    let chainToChildTokenDetailsMap: { [key: number]: { childTokenValid: boolean; childTokenAddress: string | undefined } } = {}
    ...
    if (l1Token.chainId === ChainId.MAINNET) {
      ...
      l2Chains.concat([ChainId.MAINNET]).forEach(chainId => {
        if (chainId === ChainId.MAINNET || chainToChildTokenDetailsMap[chainId].childTokenValid) {
          let extensionToAdd = omitKeyIfPresent([chainId], completeExtensions.extensions.bridgeInfo)
          const tokenInfo: TokenInfo = (chainId === ChainId.MAINNET) ? {
            ...l1Token,
            extensions: Object.keys(extensionToAdd).length > 0 ?
             {
          ...
          } as unknown as TokenInfo :
          {
            ...l1Token,
            chainId: chainId,
            address: chainToChildTokenDetailsMap[chainId].childTokenAddress,
            extensions: Object.keys(extensionToAdd).length > 0 ?
            {
            bridgeInfo: extensionToAdd
          } : undefined
...
async function generateTokenMappings(chainIds: ChainId[], l1TokenList: TokenList) {
  arbitrumMappings = chainIds.includes(ChainId.ARBITRUM_ONE) ? await getMappingProvider(
    ChainId.ARBITRUM_ONE,
    l1TokenList
  ).provide() as GenericMappedTokenData: undefined
  
   polygonMappings = chainIds.includes(ChainId.POLYGON) ? await getMappingProvider(
    ChainId.POLYGON,
    l1TokenList
  ).provide() as PolygonMappedTokenData: undefined

   optimismMappings = chainIds.includes(ChainId.OPTIMISM) ? await getMappingProvider(
    ChainId.OPTIMISM,
    l1TokenList
  ).provide() as GenericMappedTokenData: undefined
}
...
async function getChildTokenDetails(l1Token: TokenInfo, chainId: ChainId): Promise<{ childTokenValid: boolean; childTokenAddress: string | undefined }> {
